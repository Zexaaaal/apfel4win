...
if 'GSUB' in f:
    ...
    for lookup_idx, lookup in enumerate(gsub.LookupList.Lookup):
        ...
        for subtable in lookup.SubTable:
            ...
            # Check for Ligatures (actual_type 4 or Extension 4)
            if lookup.LookupType == 4 or (actual_type == 7 and subtable.ExtensionLookupType == 4):
                if hasattr(st, 'LigatureSet'):
                    for input_glyph, ligatures in st.LigatureSet.items():
                        for lig in ligatures.Ligature:
                            ...
                            print(f"  LIG: {'_'.join(comp_codes)} -> {lig_glyph}")
                            count += 1
                            if count >= 50: break
                        if count >= 50: break
                    if count >= 50: break
