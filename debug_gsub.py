...
def debug_gsub(font_path):
    ...
    for lookup in gsub.LookupList.Lookup:
        # Determine actual type (handles Extension)
        ...
        if actual_type == 4: # Ligature
            for st in subtables:
                if not hasattr(st, 'LigatureSet'): continue
                for first, ligatures in st.LigatureSet.items():
                    first_code = rev_cmap.get(first)
                    # ligatures is a LigatureSet object
                    for lig in ligatures.Ligature:
                        comp_codes = []
                        has_skin = False
                        for c in lig.Component:
                            if c in skin_gnames: has_skin = True
                            ccode = rev_cmap.get(c)
                            if ccode: comp_codes.append(f"u{ccode:04X}")
                            else: comp_codes.append(c)
                        
                        if has_skin:
                            ligs_with_skins += 1
                            if len(sample_ligs) < 20:
                                first_hex = f"u{first_code:04X}" if first_code else first
                                sample_ligs.append(f"{first_hex} + {' + '.join(comp_codes)} -> {lig.LigGlyph}")
