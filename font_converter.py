...
class EmojiConverter:
    ...
    def get_apple_sequence_map(self, font):
        ...
        def clean_seq(seq):
            return tuple(x for x in seq if x not in (65039, 8205))
        cmap = font.getBestCmap()
        for code, g in cmap.items():
            s = code,
        ...
        for gname in glyph_order:
            ...
            for p in parts:
                p_lower = p.lower()
                if p_lower.startswith('u') and all(c in '0123456789abcdef' for
                    c in p_lower[1:]):
                    sequence.append(int(p_lower[1:], 16))
                elif p_lower.startswith('uni') and all(c in
                    '0123456789abcdef' for c in p_lower[3:]):
    ...
    def convert(self, output_path):
        ...
        best_res = 160 if 160 in sbix.strikes else sorted(sbix.strikes.keys())[
            -1]
        ...
        fixed_metrics = {}
        for gname in segoe_font.getGlyphOrder():
            fixed_metrics[gname] = int(hmtx[gname][0] * scale_factor), int(
                hmtx[gname][1] * scale_factor)
        apple_pil_cache = {}
        logging.info('Processing glyphs with Permissive Mapping...')
        for gname, sequence in segoe_map.items():
            clean_segoe = tuple(x for x in sequence if x not in (65039, 8205))
            ...
            if apple_gname and apple_gname in apple_strike.glyphs:
                ...
                if not glyph_data.imageData:
                    continue
                fixed_metrics[gname] = 800, 0
                try:
                    if apple_gname not in apple_pil_cache:
                        img = Image.open(io.BytesIO(glyph_data.imageData)
                            ).convert('RGBA')
