...
def get_unicode_mapping(font):
    ...
    return {v: k for k, v in cmap.items()} # glyph_name: unicode_int
...
def compare_fonts(apple_path, segoe_path):
    ...
    # Check if 'sbix' exists and how many glyphs it covers
    sbix = apple_font['sbix']
    strike_res = max(sbix.strikes.keys())
    strike = sbix.strikes[strike_res]
    glyphs_with_bitmaps = [g for g in strike.glyphs.keys() if strike.glyphs[g].imageData is not None]
    print(f"Apple Glyphs with Bitmaps in {strike_res}px strike: {len(glyphs_with_bitmaps)}")
