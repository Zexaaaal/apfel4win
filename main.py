...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.admin_label = ctk.CTkLabel(self.main_frame, text=admin_status,
            text_color=admin_color, font=ctk.CTkFont(size=10, weight='bold'))
        self.admin_label.grid(row=0, column=2, padx=10, pady=10, sticky='ne')
        self.label = ctk.CTkLabel(self.main_frame, text=
            'Emoji Font Converter', font=ctk.CTkFont(size=24, weight='bold'))
        self.label.grid(row=0, column=0, columnspan=2, padx=20, pady=(20, 10))
        self.conv_section = ctk.CTkLabel(self.main_frame, text=
            '1. Conversion Settings', font=ctk.CTkFont(size=14, weight='bold'))
        self.conv_section.grid(row=1, column=0, columnspan=3, padx=20, pady
            =(10, 5), sticky='w')
        self.source_label = ctk.CTkLabel(self.main_frame, text=
            'Source (Apple .ttc):')
        self.source_label.grid(row=2, column=0, padx=20, pady=5, sticky='e')
        self.source_entry = ctk.CTkEntry(self.main_frame)
        self.source_entry.grid(row=2, column=1, padx=(0, 10), pady=5,
            sticky='ew')
        self.source_entry.insert(0, os.path.abspath('AppleColorEmoji.ttc'))
        self.source_button = ctk.CTkButton(self.main_frame, text='Browse',
            width=80, command=self.browse_source)
        self.source_button.grid(row=2, column=2, padx=20, pady=5)
        self.target_label = ctk.CTkLabel(self.main_frame, text=
            'Original (Segoe .ttf):')
        self.target_label.grid(row=3, column=0, padx=20, pady=5, sticky='e')
        self.target_entry = ctk.CTkEntry(self.main_frame)
        self.target_entry.grid(row=3, column=1, padx=(0, 10), pady=5,
            sticky='ew')
        self.target_entry.insert(0, os.path.abspath('seguiemj.ttf'))
        self.target_button = ctk.CTkButton(self.main_frame, text='Browse',
            width=80, command=self.browse_target)
        self.target_button.grid(row=3, column=2, padx=20, pady=5)
        self.convert_button = ctk.CTkButton(self.main_frame, text=
            'Start Conversion', command=self.start_conversion)
        self.convert_button.grid(row=4, column=0, columnspan=3, padx=20,
            pady=15, sticky='ew')
        self.inst_section = ctk.CTkLabel(self.main_frame, text=
            '2. Installation', font=ctk.CTkFont(size=14, weight='bold'))
        self.inst_section.grid(row=5, column=0, columnspan=3, padx=20, pady
            =(10, 5), sticky='w')
        self.direct_label = ctk.CTkLabel(self.main_frame, text=
            'Custom Font (optional):')
        self.direct_label.grid(row=6, column=0, padx=20, pady=5, sticky='e')
        self.direct_entry = ctk.CTkEntry(self.main_frame, placeholder_text=
            'Use generated font by default')
        self.direct_entry.grid(row=6, column=1, padx=(0, 10), pady=5,
            sticky='ew')
        self.direct_button = ctk.CTkButton(self.main_frame, text='Browse',
            width=80, command=self.browse_direct)
        self.direct_button.grid(row=6, column=2, padx=20, pady=5)
        self.install_button = ctk.CTkButton(self.main_frame, text=
            'Install to System (Requires Admin)', fg_color='green',
            hover_color='darkgreen', command=self.start_installation)
        self.install_button.grid(row=7, column=0, columnspan=3, padx=20,
            pady=15, sticky='ew')
        self.status_label = ctk.CTkLabel(self.main_frame, text=
            'Status: Ready', font=ctk.CTkFont(size=12))
        self.status_label.grid(row=8, column=0, columnspan=3, padx=20, pady=5)
        self.progress_bar = ctk.CTkProgressBar(self.main_frame)
        self.progress_bar.grid(row=9, column=0, columnspan=3, padx=20, pady
            =(0, 20), sticky='ew')
    ...
    def browse_target(self):
        filename = filedialog.askopenfilename(filetypes=[('TrueType Font',
            '*.ttf')])
        if filename:
    ...
    def on_conversion_complete(self, success):
        ...
        if success:
            ...
            self.status_label.configure(text=
                'Status: Conversion failed (Files missing?)')
            self.progress_bar.set(0)
    ...
    def start_installation(self):
        ...
        if not os.path.exists(font_to_install):
            messagebox.showerror('Error',
                f"""Font file not found: {font_to_install}
...
Please convert it first."""
        ...
        thread = threading.Thread(target=lambda : self.run_installation(
            font_to_install))
        thread.start()
    ...
    def on_installation_complete(self, success):
        ...
        if success:
            self.status_label.configure(text=
                'Status: SUCCESS! Please restart Windows.')
            messagebox.showinfo('Success',
                """Font replaced successfully!
...
Please RESTART Windows for changes to take effect."""
            ...
            messagebox.showerror('Error',
                'Failed to replace system font. Check console for errors.')
