    ...
    if 'sbix' in font:
        ...
        for res, strike in sorted(font['sbix'].strikes.items()):
            if glyph_name in strike.glyphs:
                ...
                print(
                    f'  Strike {res}px: ppem={strike.ppem}, offset=({g.originOffsetX}, {g.originOffsetY})'
                    )
            else:
                print(f'  Strike {res}px: GLYPH NOT FOUND')


g_segoe = segoe.getBestCmap().get(128512)
g_apple = apple_coll.getBestCmap().get(128512)
...
if 'CBDT' in segoe:
    print('Segoe uses CBDT/CBLC')
if 'sbix' in segoe:
    print('Segoe uses sbix')